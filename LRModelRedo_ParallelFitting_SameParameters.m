...
data2ndExp=Import["SecondExpansionData.m"][[{1,15,17,19,20},2]];
...
\[CapitalDelta]ts=(#[[2,1]]-#[[1,1]])&/@scaledData;
scaledDataFitting=Table[Join[scaledData[[dat]],Table[{scaledData[[dat,-1,1]]+i*\[CapitalDelta]ts[[dat]],maxRads[[dat]]},{i,Length@scaledData[[dat]]}]],{dat,Length@scaledData}];
...
guessNumbers=Import["guessNumbers.m"];
initialGuess=Import["initialSets.m"][[guessNumbers[[ToExpression[Environment["SLURM_ARRAY_TASK_ID"]]]]]];
...
soln=NDSolveValue[deqns/.params,z,{r,dr,\[Rho]Far},{t,0,Max@\[Tau]max},Method->{"IndexReduction"->Automatic,"EquationSimplification"->"Residual","PDEDiscretization"->{"MethodOfLines","SpatialDiscretization"->{"TensorProductGrid","MinPoints"->137,"MaxPoints"->137,"DifferenceOrder"->2}}}];

(*Check if maximum ever gets above threshold*)
maxes=Table[((\[Gamma]L*soln[dr,#])/(1+\[Gamma]L*soln[dr,#]))/.params&/@times[[dataSet]],{dataSet,Length@times}];
...
rads=
Table[If[maxes[[dataSet,i]]<0.5,0.,r/.FindRoot[(((\[Gamma]L*soln[r,times[[dataSet,i]]])/(1+\[Gamma]L*soln[r,times[[dataSet,i]]]))/.params)==.5,{r,dr,\[Rho]Far}]],
{dataSet,Length@times},{i,Length@times[[dataSet]]}];

points=Table[{times[[dataSet,i]],If[rads[[dataSet,i]]<maxRads[[dataSet]]&&i>(Length@times[[dataSet]])/2,maxRads[[dataSet]],rads[[dataSet,i]]]},{dataSet,Length@times},{i,Length@times[[dataSet]]}];
...
getSumSquare[sim_]:=Total[Flatten[Table[sim[[i,;;,2]]-scaledDataFitting[[i,;;,2]],{i,Length@scaledData}]]^2];
...
runs=ParallelTable[TimeConstrained[getRadFit[runBound[bestFit+(oa/2.^\[CapitalDelta]d)[[run]],bounds]],10,Table[ConstantArray[{0., 0.}, Length@scaledDataFitting[[i]]], {i, 
  Length@scaledDataFitting}]],{run,Length@oa}];
...
ParallelEvaluate[On[NDSolveValue::eerr]];
ParallelEvaluate[On[General::munfl]];
ParallelEvaluate[On[FindRoot::brmp]];

finalPlots=Table[Show[ListLinePlot[getRadFit[fit[[1]]][[i]],PlotRange->All,PlotStyle->Red],ListPlot[scaledData[[i]],PlotRange->All]],{i,Length@scaledDataFitting}];

Export["finalPlots_"<>ToString[Environment["SLURM_ARRAY_TASK_ID"]]<>".png",finalPlots];
Export["fits_"<>ToString[Environment["SLURM_ARRAY_TASK_ID"]]<>".m",fit];
Export["timings_"<>ToString[Environment["SLURM_ARRAY_TASK_ID"]]<>".m",timing];
